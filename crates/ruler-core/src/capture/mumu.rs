...
type NemuConnect = unsafe extern "C" fn(path: *const u16, instance_index: i32) -> i32;
...
type NemuGetDisplayId =
    unsafe extern "C" fn(handle: i32, package_name: *const u8, unknown: i32) -> i32;
...
impl CaptureBackend for MuMuController {
    fn connect(&mut self) -> Result<(), String> {
        ...
        let handle = unsafe {
            ...
            nemu_connect(root_wide.as_ptr(), self.instance_index as i32)
        };
        if handle == 0 {
            return Err(format!(
    ...
    fn capture_frame(&mut self) -> Result<CapturedFrame, String> {
        ...
        if self.spare_buffer.len() != frame_size {
            self.spare_buffer.resize(frame_size, 0);
        }
