...
class I18n:
    ...
    def _get_locale_candidates(self):
        ...
        candidates.append(os.path.join(module_dir, '..', 'locales'))
        candidates.append(os.path.join(module_dir, '..', '..', 'locales'))
        candidates.append(os.path.join(module_dir, 'timeline_tool', 'locales'))

        # PyInstaller onefile or onedir extraction path
        if hasattr(sys, '_MEIPASS') and sys._MEIPASS:
            candidates.append(os.path.join(sys._MEIPASS, 'locales'))
            candidates.append(os.path.join(sys._MEIPASS, 'timeline_tool', 'locales'))
            candidates.append(os.path.join(sys._MEIPASS, '_internal', 'locales'))
            candidates.append(os.path.join(sys._MEIPASS, '_internal', 'timeline_tool', 'locales'))

        # Relative to executable path
        executable_dir = os.path.dirname(os.path.abspath(sys.executable))
        candidates.append(os.path.join(executable_dir, 'locales'))
        candidates.append(os.path.join(executable_dir, 'timeline_tool', 'locales'))
        candidates.append(os.path.join(executable_dir, '_internal', 'locales'))
        candidates.append(os.path.join(executable_dir, '_internal', 'timeline_tool', 'locales'))

        # Current working directory
        cwd = os.path.abspath(os.getcwd())
        candidates.append(os.path.join(cwd, 'locales'))
        candidates.append(os.path.join(cwd, 'timeline_tool', 'locales'))
        candidates.append(os.path.join(cwd, '_internal', 'locales'))
        candidates.append(os.path.join(cwd, '_internal', 'timeline_tool', 'locales'))
    ...
    def __init__(self):
        ...
        logger.info("Timeline Tool I18n module initialized.")
