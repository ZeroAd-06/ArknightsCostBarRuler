...
| HUD 窗口、窗口位置/缩放、透明层 | `src/overlay.rs`, `src/slint_win.rs`, `ui/` | 保持原生窗口和 Slint 分工清晰 |
| 配置向导、截图预览、目标延迟测量 | `src/config_wizard.rs`, `src/target_discovery.rs` | 这是当前最复杂的前端热点 |
...
- `config_wizard.rs` 的 `WizardCore` 运行在单 UI 线程，靠 `Rc<RefCell<_>>` 管状态；新增并发逻辑时，先确认是否真的需要跨线程。
...
## NOTES
- 大文件热点依次是 `overlay.rs`、`target_discovery.rs`、`config_wizard.rs`、`worker.rs`；改这些文件前，先查调用链和现有注释。
