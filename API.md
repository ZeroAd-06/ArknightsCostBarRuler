...
| `totalFramesInCycle` | `integer`           | 当前费用回复循环的总帧数。新校准在边界周期可能会从基础周期 `N` 变成 `N+1`。`isRunning` 为 `false` 时是 `0`。 | `30`          | `0`         |
...
> 旧校准文件会按原来的直接映射方式读取，不会静默套用边界周期修正。重新校准后生成的新配置才会启用更精确的开区间内部点模型。
