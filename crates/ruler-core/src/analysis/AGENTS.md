...
## OVERVIEW
这里把截图帧解释成“费用条像素宽度、战斗状态、逻辑帧号、fp24 回费相位和负费状态”；这是 timing、ui scaler、校准精度问题的主战场。
...
| 校准模型、profile 读写、fp24 required 值 | `calibration.rs` | 按 `format_version` 校验，只认当前格式 |
...
## CONVENTIONS
- timing、fp24 累加器、ui scaler 相关改动，先对照 `docs/cost-bar-boundary-cycle.md`、相关录制和 calibration profile，再决定模型变更点。
...
- 校准文件按 `format_version`(当前为 4)校验，不再兼容旧/无版本格式；改 schema 时升版本号，加载失败的文件会被跳过、不进 UI。

## ANTI-PATTERNS
- 不要只靠扩大阈值或放宽判断来糊 timing 问题；先解释哪类宽度/状态被误判。
- 不要改 fp24 相位或负费行为却不同时看 replay / verifier / fixtures；这类回归通常不是单点症状。
