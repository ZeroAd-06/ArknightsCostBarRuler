...
> 当前校准文件保存 fixed-point 回费所需数值和每个循环的帧数；选用配置时会按当前分辨率和 UI 缩放动态生成费用条映射。旧版校准文件不再读取，需要重新校准生成当前格式。
> 费用条计时现在按游戏的 fixed-point 累加器模拟；常规关卡第 11 费短暂显示为 31 帧是这个模型自然推导出的结果。
