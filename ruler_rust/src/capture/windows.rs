...
unsafe extern "system" {
    fn EnumWindows(lp_enum_func: Option<unsafe extern "system" fn(HWND, LPARAM) -> BOOL>, lparam: LPARAM) -> BOOL;
...
impl WindowsController {
    pub fn new(window_handle: Option<isize>, window_title: Option<String>, window_class: Option<String>) -> Self {
        Self {
...
impl CaptureBackend for WindowsController {
    ...
    fn capture_frame(&mut self) -> Result<CapturedFrame, String> {
        let hwnd = self.hwnd.ok_or_else(|| "Windows backend is not connected".to_string())?;
        let hdc_mem = self.hdc_mem.ok_or_else(|| "Memory DC is not initialized".to_string())?;
        let hdc_window = self.hdc_window.ok_or_else(|| "Window DC is not initialized".to_string())?;
        let bmp = self.bmp.ok_or_else(|| "Capture bitmap is not initialized".to_string())?;
        ...
        unsafe {
            ...
            let mut raw = vec![0u8; row_stride * self.height as usize];
            ...
            let data = if row_stride == expected_stride {
                ...
                let mut packed = vec![0u8; expected_stride * self.height as usize];
                for row in 0..self.height as usize {
                    let src = row * row_stride;
                    let dst = row * expected_stride;
                    packed[dst..dst + expected_stride].copy_from_slice(&raw[src..src + expected_stride]);
                }
                packed
            };
